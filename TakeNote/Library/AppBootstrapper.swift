...
struct AppBootstrapper {
    ...
    static func makeModelConfiguration(debugStoreURL: @autoclosure () -> URL) -> ModelConfiguration {
        #if DEBUG
    ...
    static func bootstrapDevSchemaIfNeeded(
        ...
        do {
            try autoreleasepool {
                ...
                guard let mom = NSManagedObjectModel.makeManagedObjectModel(for: schema) else {
                    throw NSError(
                        ...
                        code: 2,
                        userInfo: [NSLocalizedDescriptionKey: "Failed to synthesize NSManagedObjectModel"]
                    )
                }
                ...
                let temp = NSPersistentCloudKitContainer(name: "TakeNoteCoreData", managedObjectModel: mom)
            ...
            if shouldIgnoreBootstrapError(error) {
                logger.info("CK bootstrap skipped due to expected condition: \(String(describing: error))")
            } else {
    ...
    private static func shouldIgnoreBootstrapError(_ error: Error) -> Bool {
        if let ck = error as? CKError {
            switch ck.code {
            case .networkUnavailable, .networkFailure, .serviceUnavailable, .notAuthenticated, .requestRateLimited, .internalError:
                return true
            default: break
            }
        }
    ...
    static func installReconciler(
        ...
        let reconciler = SystemFolderReconciler(ctx: ctx, vm: vm)
