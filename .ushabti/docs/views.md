...
- **Search:** The `.searchable` modifier binds to `takeNoteVM.searchQuery`. When `takeNoteVM.searchIsActive` is `true`, `filteredNotes` delegates to the FTS5 index (`search.index.searchNatural(query, limit: 50)`), deduplicates results by `noteID` preserving BM25 relevance order, maps to `Note` objects, and filters out Trash and Buffer notes. When search is not active, `filteredNotes` returns container notes as before (All Notes filtering excluded Trash/Buffer; other containers return their own notes). A 300 ms debounce (`@State var debounceTask`) throttles FTS queries while typing; pressing Return activates immediately via `.onSubmit(of: .search)`. Clearing the search bar calls `takeNoteVM.clearSearch()` immediately.
- **Sort:** `sortedNotes` returns `filteredNotes` unchanged when `takeNoteVM.searchIsActive` is `true` (preserves BM25 order). Date-based `sortBy`/`sortOrder` sort only applies when search is not active.
